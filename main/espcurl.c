...
int Curl_GET(char *url, char *fname, char *hdr, char *body, int hdrlen, int bodylen)
{
	...
	if (get_data.tofile) {
		if (curl_progress) {
			double curtime = 0;
			curl_easy_getinfo(curl, CURLINFO_TOTAL_TIME, &curtime);
			printf("* Download: received %lu B; time=%0.1f s; speed=%0.1f KB/sec\r\n", get_data.len, curtime, (float)(((get_data.len*10)/curtime) / 10240.0));
		}
		if (body) {
			if (strcmp(fname, "simulate") == 0) snprintf(body, bodylen, "SIMULATED save to file; size=%lu", (unsigned long)get_data.len);
			else snprintf(body, bodylen, "Saved to file %s, size=%lu", fname, (unsigned long)get_data.len);
		}
	}
...
static int _Curl_FTP(uint8_t upload, char *url, char *user_pass, char *fname, char *hdr, char *body, int hdrlen, int bodylen, uint8_t sftp)
{
	...
	if (get_data.tofile) {
		if (curl_progress) {
			...
			printf(" %lu B; time=%0.1f s; speed=%0.1f KB/sec\r\n", (unsigned long)get_data.len, curtime, (float)(((get_data.len*10)/curtime) / 10240.0));
		}

		if (body) {
			if (upload) {
				if (strcmp(fname, "simulate") == 0) snprintf(body, bodylen, "SIMULATED upload from file; size=%lu", (unsigned long)get_data.len);
				else snprintf(body, bodylen, "Uploaded file %s, size=%d", fname, fsize);
			}
			else {
				if (strcmp(fname, "simulate") == 0) snprintf(body, bodylen, "SIMULATED download to file; size=%lu", (unsigned long)get_data.len);
				else snprintf(body, bodylen, "Downloaded to file %s, size=%lu", fname, (unsigned long)get_data.len);
			}
		}
	}
